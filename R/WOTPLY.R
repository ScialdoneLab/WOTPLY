...
WOTPLY <- function(list_transition_matrices, selected_stages, cluster_label, time_label, legend_time, customize_color, top_link = NULL){
  ...
  for (i in seq(length(list_transition_matrices),1)){
    if (i == length(list_transition_matrices) ){
      ...
      transition_matrix_small <- list_transition_matrices[[i]][row.names(list_transition_matrices[[i]]) %in% selected_stages, ]
      transition_matrix_top <- select_top_weights(transition_matrix_small,top_link)
      mean_col <- apply(transition_matrix_top,2,mean)
      list_transition_matrices_top[[i]] <- transition_matrix_top[,mean_col != 0]}
    if (i != length(list_transition_matrices)){

      transition_matrix_small <- list_transition_matrices[[i]][colnames(list_transition_matrices_top[[i+1]]), ]
      transition_matrix_top <- select_top_weights(transition_matrix_small, top_link)
      mean_col <- apply(transition_matrix_top, 2, mean)
      list_transition_matrices_top[[i]] <- transition_matrix_top[, mean_col != 0]
    }
  ...
  list_color <- rep(list(0), length(list_transition_matrices)+1)
  list_time <- rep(list(0), length(list_transition_matrices)+1)
  list_label <- rep(list(0), length(list_transition_matrices)+1)

  for (i in seq(length(list_color), 1)){
    if (i != length(list_color)){
      list_color[[i]] <- map_color[colnames(list_transition_matrices_top[[i]]), 2]
    }
    if (i == length(list_color)){
      list_color[[i]] <- map_color[row.names(list_transition_matrices_top[[i-1]]), 2]
    }
  }
  ...
  for (i in seq(length(list_time), 1)){
    ...
    if (i == length(list_time)){
      list_time[[i]] <- rep(legend_time[i], length(row.names(list_transition_matrices_top[[i-1]])))
    }
  }
  ...
  for (i in seq(length(list_label), 1)){
    ...
    if (i == length(list_label)){
      list_label[[i]] <- row.names(list_transition_matrices_top[[i-1]])
    }
  }
  ...
  for (i in 1:length(unisco_all)){
    ...
    if (i != 1){
      unisco_all[[i]][day_all_small == legend_time[i-1]] <- 0.1
    }
  }
  ...
  for (i in 1:length(unisco_all_complete)){
    unisco_all_complete[[i]] <- rep(list(0), sum(day_all_small == legend_time[i]))
  }
  ...
  for (i in 1:length(unisco_all_complete)){
    ...
    for ( j in 1:sum(day_all_small == time_point)){
      ...
      if (i != 1){
        unisco_all_complete[[i]][[j]][unisco_all[[i]] == 0.1] <- as.numeric(list_transition_matrices_top[[i-1]][j, ])
      }
    }
  }
  ...
  for (i in 1:length(coordinate_x)){
    coordinate_x[[i]] <- rep(position_x[[i]], sum(day_all_small == legend_time[i]))
  }
  day_x_all <- unlist(coordinate_x)

  coordinate_y <- rep(list(0),length(legend_time))
  for (i in 1:length(coordinate_y)){
    coordinate_y[[i]] <- seq(1, 10, length.out=sum(day_all_small == legend_time[i]))
  }
  ...
  matrix_position <- matrix(all_valori, nrow =length(colnames(provo)), ncol=2)

  pesi_final <- rep(list(0),length(list_transition_matrices_top))
  for (i in 1:length(pesi_final)){
    pesi_final[[i]] <- as.numeric(as.matrix(list_transition_matrices_top[[i]]))[as.numeric(as.matrix(list_transition_matrices_top[[i]])) != 0]
  }
  ...
  #pdf("/Users/gabriele.lubatti/Desktop/Phd/Embryo_Organoids/paper_text_figures/figures/connection_plot/connection_top_3_new.pdf",width = 8, height = 6)
  GGally::ggnet2(bip, mode = matrix_position,size = 4, color = color_all_small, label = time_all_small, edge.size = pesi_new_small, arrow.size = 12, arrow.gap = 0.025)
